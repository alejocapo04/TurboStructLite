...
DEFINE_FUNCTION(UTurboStructLiteQueryLibrary::execTurboStructLiteLoadStructLogic)
{
	...
	if (!DataProp || !DataPtr)
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadStructLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a struct"), FString(), FDateTime(0), SaveVersion, FString());
		return;
	}
	FString TrimmedQuery = QueryString;
	TrimmedQuery.TrimStartAndEndInline();
	const bool bSelectMode = TrimmedQuery.StartsWith(TEXT("SELECT"), ESearchCase::IgnoreCase);
	if (!bSelectMode && !CastField<FStructProperty>(DataProp))
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadStructLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a struct"), FString(), FDateTime(0), SaveVersion, FString());
		return;
	...
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadStructLogic: Invalid slot parameters"), ELogVerbosity::Error);
		...
		auto RunSelectLoad = [SlotCopy, SubSlotIndex, QueryCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, EncryptionKeyCopy, SelectedEncryption, ClampedParallel, SaveVersion](bool bApplyOnGameThread) mutable
		{
			...
			auto ApplyResults = [SlotCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, bSuccess, ErrorMessage, Metadata, SaveDate, SaveVersion, StatsText, CombinedValue = MoveTemp(CombinedValue)]() mutable
			{
				...
				if (!DataProp || !DataPtr)
				{
					if (bUseWriteAheadLog)
					{
						UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, TEXT("LoadLogic skipped: invalid target pointer"));
					}
					FTurboStructLiteLogicLoadComplete Local = LoadDelegate;
				...
				if (bUseWriteAheadLog)
				{
					UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, bApplied && bSuccess ? TEXT("LoadLogic completed") : TEXT("LoadLogic failed"));
		...
		auto Task = [RunSelectLoad, bAsync, bCanBackground]() mutable
		{
			if (bAsync)
			{
				if (bCanBackground)
				{
					...
					{
#if WITH_EDITOR
						TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadLogic_AsyncTask"));
#endif
						RunSelectLoad(true);
					});
				}
...
#if WITH_EDITOR
			TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadLogic_Sync"));
#endif
			RunSelectLoad(false);
		};
					...
					else
					{
						...
						ParallelFor(TaskCount, [&](int32 TaskIndex)
						{
							...
							for (int32 SubSlotIdx = Start; SubSlotIdx < End; ++SubSlotIdx)
							{
								...
								if (bUseWriteAheadLog)
								{
									UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("LoadLogic SubSlot=%d"), CurrentSubSlot));
								}
...
DEFINE_FUNCTION(UTurboStructLiteQueryLibrary::execTurboStructLiteLoadSetLogic)
{
	P_GET_PROPERTY(FStrProperty, MainSlotName);
	P_GET_PROPERTY(FIntProperty, SubSlotIndex);
	P_GET_UBOOL(bAsync);
...
auto EmitResult = [LoadDelegate, MainSlotName, SubSlotIndex](bool bSuccess, const FString& ErrorMessage, const FString& Metadata, const FDateTime& SaveDate, int32 SaveVersion, const FString& StatsText) mutable
{
	...
	if (!DataProp || !DataPtr)
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadSetLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a set"), FString(), FDateTime(0), SaveVersion, FString());
		return;
	}
	FString TrimmedQuery = QueryString;
	TrimmedQuery.TrimStartAndEndInline();
	const bool bSelectMode = TrimmedQuery.StartsWith(TEXT("SELECT"), ESearchCase::IgnoreCase);
	if (!bSelectMode && !CastField<FSetProperty>(DataProp))
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadSetLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a set"), FString(), FDateTime(0), SaveVersion, FString());
		return;
...
if (MainSlotName.IsEmpty() || SubSlotIndex < -1)
{
	FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadSetLogic: Invalid slot parameters"), ELogVerbosity::Error);
	EmitResult(false, TEXT("IO Error: Invalid slot parameters"), FString(), FDateTime(0), SaveVersion, FString());
	return;
}
...
if (bSelectMode)
{
	...
	if (bUseWriteAheadLog)
	{
		const FString LoadOpLabel = FString::Printf(TEXT("LoadSetLogic_Enc%d"), static_cast<int32>(ResolvedEncryption));
		WALPath = UTurboStructLiteBPLibrary::GenerateWALPath(MainSlotName, SubSlotIndex, LoadOpLabel);
		UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("Queued LoadSetLogic Slot=%s SubSlot=%d Async=%s Encryption=%d Queue=%d Threads=%d Batching=%d"), *MainSlotName, SubSlotIndex, bAsync ? TEXT("true") : TEXT("false"), static_cast<int32>(ResolvedEncryption), ClampedPriority, ClampedParallel, static_cast<int32>(CompressionBatching)));
		UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("Query=%s"), *QueryString));
	}
	...
	auto RunSelectLoad = [SlotCopy, SubSlotIndex, QueryCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, EncryptionKeyCopy, SelectedEncryption, ClampedParallel, SaveVersion](bool bApplyOnGameThread) mutable
	{
		...
		auto ApplyResults = [SlotCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, bSuccess, ErrorMessage, Metadata, SaveDate, SaveVersion, StatsText, CombinedValue = MoveTemp(CombinedValue)]() mutable
		{
			...
			if (!DataProp || !DataPtr)
			{
				if (bUseWriteAheadLog)
				{
					UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, TEXT("LoadSetLogic skipped: invalid target pointer"));
				}
				FTurboStructLiteLogicLoadComplete Local = LoadDelegate;
				Local.ExecuteIfBound(false, TEXT("Type Error: Invalid target pointer"), FString(), FDateTime(0), SaveVersion, FString());
				UTurboStructLiteBPLibrary::FinishQueuedLoad(SlotCopy);
			...
			if (bUseWriteAheadLog)
			{
				UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, bApplied && bSuccess ? TEXT("LoadSetLogic completed") : TEXT("LoadSetLogic failed"));
				if (bApplied && bSuccess)
				{
					UTurboStructLiteBPLibrary::DeleteWALFile(WALPath);
				}
	...
	auto Task = [RunSelectLoad, bAsync, bCanBackground]() mutable
	{
		if (bAsync)
		{
			if (bCanBackground)
			{
				...
				{
#if WITH_EDITOR
					TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadSetLogic_AsyncTask"));
#endif
					RunSelectLoad(true);
				});
			}
...
#if WITH_EDITOR
		TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadSetLogic_Sync"));
#endif
		RunSelectLoad(false);
	};
					...
					else
					{
						...
						ParallelFor(TaskCount, [&](int32 TaskIndex)
						{
							...
							for (int32 SubSlotIdx = Start; SubSlotIdx < End; ++SubSlotIdx)
							{
								...
								if (bUseWriteAheadLog)
								{
									UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("LoadSetLogic SubSlot=%d"), CurrentSubSlot));
								}
								...
								else
								{
									...
									if (UTurboStructLiteBPLibrary::GetRootMetaTypeFromBytes(RawBytes, RootType))
									{
										...
										if (NormalRoot != NormalData)
										{
											if (FSetProperty* SetProp = CastField<FSetProperty>(DataProp))
											{
												const FString NormalInner = NormalizeType(SetProp->ElementProp->GetCPPType(nullptr, 0));
												if (NormalRoot == NormalInner)
												{
													bSingleElement = true;
									...
									if (bLocalSuccess)
									{
										if (bSingleElement && ElementProp)
										{
											...
											else
											{
												...
												if (bMatch)
												{
													FSetProperty* SetProp = CastField<FSetProperty>(DataProp);
													if (SetProp)
													{
														FScriptSetHelper SubHelper(SetProp, LocalValue.GetData());
														SubHelper.AddElement(ElementValue.GetData());
													}
												}
												ElementProp->DestroyValue(ElementValue.GetData());
											}
...
DEFINE_FUNCTION(UTurboStructLiteQueryLibrary::execTurboStructLiteLoadMapLogic)
{
	P_GET_PROPERTY(FStrProperty, MainSlotName);
	P_GET_PROPERTY(FIntProperty, SubSlotIndex);
	P_GET_UBOOL(bAsync);
...
auto EmitResult = [LoadDelegate, MainSlotName, SubSlotIndex](bool bSuccess, const FString& ErrorMessage, const FString& Metadata, const FDateTime& SaveDate, int32 SaveVersion, const FString& StatsText) mutable
{
	...
	if (!DataProp || !DataPtr)
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadMapLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a map"), FString(), FDateTime(0), SaveVersion, FString());
		return;
	}
	FString TrimmedQuery = QueryString;
	TrimmedQuery.TrimStartAndEndInline();
	const bool bSelectMode = TrimmedQuery.StartsWith(TEXT("SELECT"), ESearchCase::IgnoreCase);
	if (!bSelectMode && !CastField<FMapProperty>(DataProp))
	{
		FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadMapLogic: Invalid data type"), ELogVerbosity::Error);
		EmitResult(false, TEXT("Type Error: Data must be a map"), FString(), FDateTime(0), SaveVersion, FString());
		return;
...
if (MainSlotName.IsEmpty() || SubSlotIndex < -1)
{
	FFrame::KismetExecutionMessage(TEXT("TurboStructLiteLoadMapLogic: Invalid slot parameters"), ELogVerbosity::Error);
	EmitResult(false, TEXT("IO Error: Invalid slot parameters"), FString(), FDateTime(0), SaveVersion, FString());
	return;
}
...
if (bSelectMode)
{
	...
	if (bUseWriteAheadLog)
	{
		const FString LoadOpLabel = FString::Printf(TEXT("LoadMapLogic_Enc%d"), static_cast<int32>(ResolvedEncryption));
		WALPath = UTurboStructLiteBPLibrary::GenerateWALPath(MainSlotName, SubSlotIndex, LoadOpLabel);
		UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("Queued LoadMapLogic Slot=%s SubSlot=%d Async=%s Encryption=%d Queue=%d Threads=%d Batching=%d"), *MainSlotName, SubSlotIndex, bAsync ? TEXT("true") : TEXT("false"), static_cast<int32>(ResolvedEncryption), ClampedPriority, ClampedParallel, static_cast<int32>(CompressionBatching)));
		UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("Query=%s"), *QueryString));
	}
	...
	auto RunSelectLoad = [SlotCopy, SubSlotIndex, QueryCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, EncryptionKeyCopy, SelectedEncryption, ClampedParallel, SaveVersion](bool bApplyOnGameThread) mutable
	{
		...
		auto ApplyResults = [SlotCopy, DataProp, DataPtr, LoadDelegate, bUseWriteAheadLog, WALPath, bSuccess, ErrorMessage, Metadata, SaveDate, SaveVersion, StatsText, CombinedValue = MoveTemp(CombinedValue)]() mutable
		{
			...
			if (!DataProp || !DataPtr)
			{
				if (bUseWriteAheadLog)
				{
					UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, TEXT("LoadMapLogic skipped: invalid target pointer"));
				}
				FTurboStructLiteLogicLoadComplete Local = LoadDelegate;
				Local.ExecuteIfBound(false, TEXT("Type Error: Invalid target pointer"), FString(), FDateTime(0), SaveVersion, FString());
				UTurboStructLiteBPLibrary::FinishQueuedLoad(SlotCopy);
			...
			if (bUseWriteAheadLog)
			{
				UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, bApplied && bSuccess ? TEXT("LoadMapLogic completed") : TEXT("LoadMapLogic failed"));
				if (bApplied && bSuccess)
				{
					UTurboStructLiteBPLibrary::DeleteWALFile(WALPath);
				}
	...
	auto Task = [RunSelectLoad, bAsync, bCanBackground]() mutable
	{
		if (bAsync)
		{
			if (bCanBackground)
			{
				...
				{
#if WITH_EDITOR
					TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadMapLogic_AsyncTask"));
#endif
					RunSelectLoad(true);
				});
			}
...
#if WITH_EDITOR
		TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("TurboStructLite_LoadMapLogic_Sync"));
#endif
		RunSelectLoad(false);
	};
					...
					else
					{
						...
						ParallelFor(TaskCount, [&](int32 TaskIndex)
						{
							...
							for (int32 SubSlotIdx = Start; SubSlotIdx < End; ++SubSlotIdx)
							{
								...
								if (bUseWriteAheadLog)
								{
									UTurboStructLiteBPLibrary::WriteWALEntry(WALPath, FString::Printf(TEXT("LoadMapLogic SubSlot=%d"), CurrentSubSlot));
								}
